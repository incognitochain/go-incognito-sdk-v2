...
func (server *RPCServer) CreateAndSendTransactionFromAToB(privKeyA string, paymentAddress string, amount string) ([]byte, error) {
	if len(server.GetURL()) == 0 {
		return []byte{}, errors.New("Server has not set mainnet or testnet")
	}
...
func (server *RPCServer) CreateAndSendPrivacyCustomTokenTransaction(privKeyStr, tokenName string) ([]byte, error) {
...
func (server *RPCServer) TransferPrivacyCustomToken(privKeyStrA string, paymentAddress string, tokenID string, amount string) ([]byte, error) {
...
func (server *RPCServer) GetBalancePrivacyCustomToken(privKeyStr string, tokenID string) ([]byte, error) {
	query := fmt.Sprintf(`{
		"id": 1,
		"jsonrpc": "1.0",
...
func (server *RPCServer) SwitchTokenCoinVersion(privKey string, tokenID string) ([]byte, error) {
	query := fmt.Sprintf(`{
		"jsonrpc": "1.0",
...
func (server *RPCServer) SwitchCoinVersion(privKey string) ([]byte, error) {
	query := fmt.Sprintf(`{
		"jsonrpc": "1.0",
