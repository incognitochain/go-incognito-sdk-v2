...
func CommitteePublicKeyFromPrivateKey(privateKey, privateSeed string) (string, error) {
	senderWallet, err := wallet.Base58CheckDeserialize(privateKey)
	if err != nil {
		return "", err
	}

	pk := senderWallet.KeySet.PaymentAddress.Pk
	if len(pk) == 0 {
		return "", fmt.Errorf("candidate payment address invalid: %v", senderWallet.Base58CheckSerialize(wallet.PaymentAddressType))
	}

	seed, _, err := base58.Base58Check{}.Decode(privateSeed)
	if err != nil {
		return "", fmt.Errorf("cannot decode private seed: %v", privateSeed)
	}

	committeePK, err := key.NewCommitteeKeyFromSeed(seed, pk)
	if err != nil {
		return "", fmt.Errorf("cannot create committee key from pk: %v, seed: %v. Error: %v", pk, seed, err)
	}
	return committeePK.ToBase58()
}
